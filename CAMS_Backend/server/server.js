...
app.use(cors({
  origin: [
    ...
    'https://fypbnplbackend.vercel.app'
  ],
  credentials: true,
...
app.get('/users/booklog', async (req, res) => {
  ...
  if (row.timestamp) {
    // 1. Create a date object from the database timestamp
    const date = new Date(row.timestamp);
    
    // 2. Convert to Malaysia Time string (Asia/Kuching)
    // This handles the +8 hours automatically and correctly
    row.timestamp = date.toLocaleString('en-GB', { 
      ...
      second: '2-digit',
      hour12: false 
    }).replace(',', ''); // format: DD/MM/YYYY HH:mm:ss
  }
  return row;
});
    } else {
...
//BNPL system
app.post('/bnpl/payment', async (req, res) => {
  const { userid, amount, months, productName } = req.body;
  let client;
  ...
  try {
    if (!userid || !amount || !months) {
      return res.status(400).json({
        success: false,
        message: 'userid, amount, and months are required'
      });
    }
    ...
    if (isNaN(numericAmount) || isNaN(numericMonths)) {
      return res.status(400).json({
        success: false,
        message: 'Invalid amount or months'
      });
    }
    ...
    const monthlyPayment = numericAmount / numericMonths;
    const timestamp = new Date();
    const newServiceId = `bnpl_${Date.now()}`;
    ...
    return res.json({
      success: true,
      message: 'BNPL payment recorded successfully'
    });
